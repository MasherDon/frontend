    ...
    <div v-if="disabled">
      <form class="yy" onsubmit="return false">
        ...
        <textarea cols="80" rows="20" placeholder="Содержание"  v-model="form.body" required>
          ...
          <input v-on:keyup.enter="strok()" type="text" placeholder="Теги" v-model="teg"/>
          <div class="hh">
...
export default {
  ...
  methods: {
    kok() {
      this.form.title = this.put['title']
      this.form.description = this.put['description']
      this.form.body = this.put['body']
      this.form.tagList = this.put['tagList']
    },
    async sot() {
      try {
